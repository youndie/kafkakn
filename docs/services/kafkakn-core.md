...
| `linuxArm64` | built and run on request (`-Pkafkakn.linuxArm64`), not published | same as `linuxX64`; the bundle is built on an arm64 Docker, with OpenSSL seeded from `/dev/urandom`; binaries need glibc 2.17 | [§2.28](../research/research-architecture.md) — run on arm64 hardware by `ci/b-39/run.sh`; not published and not run in CI, by decision |
