...
| **`linuxArm64` as a shipped target** | not published. It is built on request (`-Pkafkakn.linuxArm64`) and run on arm64 hardware since [B-39](docs/backlog/B-39-linux-arm64.md): the native suite passes there and agrees with the JVM arm (93 tests, 17 observations, measured 2026-09-25). Its binaries need glibc 2.25, not 2.17 ([B-44](docs/backlog/B-44-arm64-glibc-floor.md)) |
