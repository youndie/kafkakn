...
jvm_member() { # <group> <topic> [leave-ms]
    ...
    env KAFKAKN_REBALANCE_GROUP="$1" KAFKAKN_REBALANCE_TOPIC="$2" KAFKAKN_REBALANCE_PARTITIONS=$PARTITIONS \
        KAFKAKN_REBALANCE_END=$((RECORDS / PARTITIONS)) ${3:+KAFKAKN_REBALANCE_LEAVE_MS=$3} \
        ./gradlew --console=plain :kafkakn-core:jvmTest --rerun --tests '*RebalanceListenerTest.a_member*'
}
native_member() { # <group> <topic> [leave-ms]
...
round() { # <stayer-arm> <leaver-arm>
    ...
    group=$topic
    echo
    echo "=== $stayer stays, $leaver leaves after 15 s ==="
    PARTITIONS=$PARTITIONS bash "$H" topic "$topic" >/dev/null
    rm -rf "$OBS"
    # The stayer first. The writer starts with the leaver, and writes for about fifty seconds, so the
    # leaver joins, takes a share, processes some of it, and hands it back mid-stream.
    ...
    local trickle=$!
    "${leaver}_member_run" "$group" "$topic" 15000 > "build/b-50-$leaver-leaves.out" 2>&1
    local leaver_code=$?
    wait "$trickle"
    wait "$stayer_pid"
    local stayer_code=$?
